...
@section('dashboard-content')
    <div class="card">
        <div class="card-title text-dark mb-3">
            <b>Add a new preorder item</b>
            <hr>
            <a href="{{ route('order.preOrderItem') }}">
                <button class="btn btn-sm btn-dark px-3">
                    <i class="fa fa-plus" aria-hidden="true"></i> new preorder product
                </button>
            </a>
        </div>
    </div>
            ...
            <tbody>
                @foreach ($products as $product)
                    <tr>
                        ...
                        <td>
                            <a href="{{ route('preorder.edit', $product->id) }}" class="text-dark">
                        ...
                        <td>{{ $outstock_sell }}</td>
                        <td>
                            <a href="{{ route('preorder.config.variant', $product->id) }}"
                                class="btn btn-dark btn-sm px-3">configure
                                <i class="fa fa-cog" aria-hidden="true"></i>
                            </a>
                        </td>
                        <td>
                            <a href="{{ route('preorder.delete', $product->id) }}" class="text-dark text-lg">
